...
class GRN(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        gx = torch.norm(x, p=2, dim=(2, 3), keepdim=True)     # L2 over spatial dims                                                                                                                           # 哔哩哔哩/微信公众号: CV缝合救星, 独家整理!
        nx = x / (gx + self.eps)
...
class PFGA(nn.Module):
    ...
    class Branch(nn.Module):
        def __init__(self, dim: int, K: int, center_suppress: bool = True):
            ...
            self.dw_h = nn.Conv2d(dim, dim, kernel_size=(1, K),
                                  padding=(0, K // 2), groups=dim, bias=False)                                                                                                                           # 哔哩哔哩/微信公众号: CV缝合救星, 独家整理!
            self.dw_v = nn.Conv2d(dim, dim, kernel_size=(K, 1),                                                                                                                           # 哔哩哔哩/微信公众号: CV缝合救星, 独家整理!
    ...
    def __init__(self, dim: int, K_list=(9, 15, 31), use_grn: bool = False, center_suppress: bool = True):                                                                                                                           # 哔哩哔哩/微信公众号: CV缝合救星, 独家整理!
        super().__init__()
        self.dim = dim
        self.K_list = K_list

        # multi-scale peripheral branches
        self.branches = nn.ModuleList([PFGA.Branch(dim, K, center_suppress=center_suppress) for K in K_list])                                                                                                                           # 哔哩哔哩/微信公众号: CV缝合救星, 独家整理!
