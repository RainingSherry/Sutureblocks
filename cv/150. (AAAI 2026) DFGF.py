...
class FFT_Process(nn.Module):
    def __init__(self, nf):
        ...
        self.feature_fusion = nn.Conv2d(nf * 2, nf, kernel_size=1, stride=1, padding=0)                                                                                                                           # 哔哩哔哩/微信公众号: CV缝合救星, 独家整理!
...
def create_wavelet_filter(wave, in_size, out_size, type=torch.float):
    ...
    dec_filters = torch.stack([dec_lo.unsqueeze(0) * dec_lo.unsqueeze(1),
                               ...
                               dec_hi.unsqueeze(0) * dec_hi.unsqueeze(1)], dim=0)                                                                                                                           # 哔哩哔哩/微信公众号: CV缝合救星, 独家整理!
    ...
    rec_filters = torch.stack([rec_lo.unsqueeze(0) * rec_lo.unsqueeze(1),
                               ...
                               rec_hi.unsqueeze(0) * rec_hi.unsqueeze(1)], dim=0)                                                                                                                           # 哔哩哔哩/微信公众号: CV缝合救星, 独家整理!
...
class WTConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=5, stride=1, bias=True, wt_levels=1, wt_type='db1'):
        ...
        self.wavelet_convs = nn.ModuleList(
            [nn.Conv2d(in_channels*4, in_channels*4, kernel_size, padding='same', stride=1, dilation=1, groups=in_channels*4, bias=False) for _ in range(self.wt_levels)]                                                                                                                           # 哔哩哔哩/微信公众号: CV缝合救星, 独家整理!
        )
...
class SpatialAugmentModel(nn.Module):
    def __init__(self, nf=16):
        ...
        self.spatialaugment2 = DeformableDilatedConv(in_channels=nf, out_channels=nf, dilation=2)                                                                                                                           # 哔哩哔哩/微信公众号: CV缝合救星, 独家整理!
        # 输出卷积：nf 通道 -> 3 通道
...
class MultiConvBlock(nn.Module):
    def __init__(self, dim, num_heads=4, expand_ratio=2):
        ...
        self.conv_reduction = nn.Conv2d(dim, dim // 4, kernel_size=1, stride=1, bias=True)                                                                                                                           # 哔哩哔哩/微信公众号: CV缝合救星, 独家整理!
    ...
    def forward(self, x):
        ...
        for conv in self.local_convs:
            x_scale = self.leakyrelu(conv(x_reduced))  # Apply multi-scale convolution                                                                                                                           # 哔哩哔哩/微信公众号: CV缝合救星, 独家整理!
...
class ChannelAttentionFusion(nn.Module):
    ...
    def forward(self, fft_features, multi_features):
        ...
        fft_weight, multi_weight = torch.split(attention_weights, fft_features.size(1), dim=1)                                                                                                                           # 哔哩哔哩/微信公众号: CV缝合救星, 独家整理!
...
class DFGFLow(nn.Module):
    def __init__(self, nf=16, numblocks = 6):
        ...
        self.s_nf = nf
        self.processblock = LowFrequencyProcessing(nf=self.s_nf, num_blocks=numblocks, input_channels=3)                                                                                                                           # 哔哩哔哩/微信公众号: CV缝合救星, 独家整理!
