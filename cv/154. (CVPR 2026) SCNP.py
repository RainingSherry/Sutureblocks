...
class SCNP(torch.nn.Module):
    ...
    def forward(self, logits, target):
        assert logits.shape == target.shape, "`target` should be one-hot encoded and have the same shape as `logits`"                                                                                                                           # 哔哩哔哩/微信公众号: CV缝合救星, CVFenghe 独家整理!

        # MinPooling in the foreground
        t1 = -self.mp(-(logits*target+9999*(1-target)), self.ns, self.st, self.pad)                                                                                                                           # 哔哩哔哩/微信公众号: CV缝合救星, CVFenghe 独家整理!
        # MaxPooling in the background
        t2 = self.mp((logits*(1-target)-9999*target), self.ns, self.st, self.pad)                                                                                                                           # 哔哩哔哩/微信公众号: CV缝合救星, CVFenghe 独家整理!
