...
# 傅里叶门控残差通道注意力 (FG-RCA)
class FourierGatedResidualChannelAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        spec = torch.fft.fft2(grid)                 # 复数张量 (B, 1, Hc, Wc)
        amp = torch.abs(spec)
...
if __name__ == "__main__":
    ...
    module = FourierGatedResidualChannelAttention(channels=C, up_scale=1) # 由于 DNRU 上采样 ×2，此处暂时不设置上采样即up_scale=1, 保证输入输出大小一致
    y = module(x)
