...
class CEB(nn.Module):
    def __init__(self, num_feat):
        ...
        self.mlp1 = nn.Sequential(
            nn.Conv2d(in_channels=num_feat // 2, out_channels=num_feat // 2, kernel_size=1, padding=0),                                                                                                                               # 哔哩哔哩/微信公众号: CV缝合救星, 独家整理!
            nn.ReLU(inplace=True),
...
if __name__ == "__main__":
    ...
    # 创建一个随机输入特征图 
    input_tensor = torch.randn(1, 32, 256, 256).to(device)    # (batch_size, channels, height, width)                                                                                                                               # 哔哩哔哩/微信公众号: CV缝合救星, 独家整理!
