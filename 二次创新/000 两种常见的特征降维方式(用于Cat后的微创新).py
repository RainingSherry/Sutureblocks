...
class ChannelReducer(nn.Module):
    def __init__(self, channel_num):
        ...
        """
            注意：根据降维的程度进行修改
        """
        self.feature_fuser = nn.Linear(4 * channel_num, channel_num)  # 定义融合层，用于融合来自不同方向的信息

        self.conv_bn_activation = nn.Sequential(
            # 添加2D卷积层，输入通道数是原通道数的3倍，输出通道数为channel_num
            nn.Conv2d(4 * channel_num, channel_num, kernel_size=1, stride=1),
...
class ChannelReducer(nn.Module):
    def __init__(self, channel_num):
        ...
        self.conv_bn_activation = nn.Sequential(
            nn.Conv2d(4 * channel_num, channel_num, kernel_size=1, stride=1),  # 输入4倍通道数
            nn.BatchNorm2d(channel_num),
            nn.ReLU(inplace=True),
        )

    def forward(self, x1, x2, x3, x4):
        merged_features = torch.cat([x1, x2, x3, x4], dim=1)
        output_tensor = self.conv_bn_activation(merged_features)
        return output_tensor
